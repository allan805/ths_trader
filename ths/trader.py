...
class ThsTrader:
    ...
    def input_text(self, text: str, instance: int = 0) -> bool:
        ...
        if edit.exists:
            ...
            edit.set_text(text)
            logger.info(
                f"输入文本成功: instance={instance}"
            )
            return True
        return False

    @timer
    def input_stock_code(self, code: str) -> bool:

        """使用 ADB 键盘输入股票代码，触发同花顺股票匹配。"""
        code = str(code).strip()
        ...
        try:
            ...
            # 取得 ADB 设备
            serial = getattr(config, "DEVICE_SERIAL", None)
            ...
            if result.returncode != 0:
                logger.error("❌ ADB 输入股票代码失败")
                return False

            logger.info("✅ 股票代码输入完成")
            return True

        except Exception as e:
            logger.error(f"❌ 股票代码输入异常: {e}")
            return False
    ...
    def input_stock_code(self, code: str) -> bool:
        ...
        """
        使用 Android ADB 键盘输入股票代码。
        
        同花顺的股票代码输入框需要真实键盘事件才能触发
        股票匹配；uiautomator2.set_text() 虽然能改变显示文字，
        但不会可靠触发同花顺的股票识别。
        """
        code = str(code).strip()
        ...
        try:
            # 点击股票代码输入框
            edit = self.d(resourceId="com.hexin.plat.android:id/auto_stockcode")
            ...
            # 使用 adb shell input text，模拟真实键盘输入
            serial = getattr(config, "DEVICE_SERIAL", None)
            ...
            cmd += ["shell", "input", "text", code]

            logger.info("通过 Android 键盘输入股票代码")
            ...
            if result.returncode != 0:
                logger.error("❌ ADB 输入股票代码失败")
                if result.stderr:
                    logger.error("ADB错误: %s", result.stderr.strip())
                return False
    ...
    def go_to_trade(self) -> bool:
        ...
        while time.time() < deadline:
            ...
            # -------------------------------------------------
            # 优先直接定位底部“交易”
            # 不使用 click_by_text(timeout=5)，避免长时间等待。
            # -------------------------------------------------

            try:
                trade_tab = self.d(text="交易")
    ...
    def _verify_stock_selection(self, code: str, timeout: float = 5.0) -> bool:
        """
        验证同花顺是否真正识别股票。

        注意：
        「股票名称」是默认占位文字，不能认为股票已经识别。
        """
        ...
        while time.time() < deadline:
            try:
                ...
                if code_obj.exists:
                    actual_code = (code_obj.get_text() or "").strip()

                if name_obj.exists:
                    stock_name = (name_obj.get_text() or "").strip()
                ...
                logger.info(
                    "🔎 股票识别状态: "
                    f"代码={actual_code}, 名称={stock_name}"
                )
    ...
    def _execute_order(
        ...
        try:
            ...
            if not self.input_stock_code(str(code)):
                raise Exception(
                    "无法输入股票代码"
                )
            ...
            # 8.5 实盘最终确认
            # ==================================================
            if side == "buy" and self.trade_mode != "simulate":
                ...
                if not confirm_ok.wait(timeout=5):
                    raise Exception(
                        "未出现「确认买入」按钮，"
                        "禁止提交实盘委托"
                    )
                ...
                logger.info(
                    "🔎 最终委托确认："
                    f"股票={confirm_code} "
                    f"数量={confirm_quantity} "
                    f"价格={confirm_price}"
                )

                expected_code = str(code).strip()
                expected_quantity = str(quantity).strip()
                expected_price = str(price).strip()
                ...
                if confirm_quantity != expected_quantity:
                    raise Exception(
                        "实盘确认失败：数量不一致，"
                        f"预期={expected_quantity}，"
                        f"实际={confirm_quantity}"
                    )

                if confirm_price != expected_price:
                    raise Exception(
                        "实盘确认失败：价格不一致，"
                        f"预期={expected_price}，"
                        f"实际={confirm_price}"
            ...
            if trade_result.get("success"):
                logger.info(
                    f"✅ {side_name}页面检测到成功反馈，"
                    f"继续查询当日委托确认"
                )
            else:
                logger.warning(
                    f"⚠️ {side_name}页面未检测到明确成功反馈，"
                    f"但也未发现失败信息，"
                    f"继续查询当日委托确认"
                )
